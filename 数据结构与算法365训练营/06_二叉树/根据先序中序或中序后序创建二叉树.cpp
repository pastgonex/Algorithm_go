...
BTree pre_mid_createBTree(char *pre, char *mid, int len) //前序中序还原建立二叉树
{
    if (len == 0)
        return NULL;
    ...
    BTree T = new BNode; //创建根结点
    T->data = ch;
    T->lchild = pre_mid_createBTree(pre + 1, mid, index);                               //建立左子树
    T->rchild = pre_mid_createBTree(pre + index + 1, mid + index + 1, len - index - 1); //建立右子树
    return T;
}

BTree pro_mid_createBTree(char *last, char *mid, int len) //后序中序还原建立二叉树
{
    if (len == 0)
        return NULL;
    ...
    BTree T = new BNode; //创建根结点
    T->data = ch;
    T->lchild = pro_mid_createBTree(last, mid, index);                               //建立左子树
    T->rchild = pro_mid_createBTree(last + index, mid + index + 1, len - index - 1); //建立右子树
    return T;
}
