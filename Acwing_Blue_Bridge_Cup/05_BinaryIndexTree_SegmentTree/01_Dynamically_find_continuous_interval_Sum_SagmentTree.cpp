...
todo
 线段树的建树
    build:
        给定一个长度为N的序列A, 我们可以在区间 [1,N]上建立一颗线段树,
每个月节点[i,i]保存A[i]的值 线段树的二叉树结构, 可以很方便地从下往上传递信息
    以区间中所有数的和为例:
        记 dat(l,r) 等于sum(l,r){A[i]}  (l<=i<=r),
显然dat(l,r)=sum(dat(l,mid),dat(mid+1,r))
    *****************************************************************************************
    以区间中所有数的最大值问(区间最大值)题为例:
        记dat(l,r)等于max(l<=i<=r){A[i]}, 显然
dat(l,r)=max(dat(l,mid),dat(mid+1,r))
    *****************************************************************************************
    调用入口都是 build(1,1,n);
    *****************************************************************************************
...
todo
 线段树的单点修改
    change/modify:
        ...
        我们需要从根节点出发, 递归找到代表区间[x,x]的叶节点,
然后从下往上更新[x,x]以及他的所有 祖先节点上保存的信息, 时间复杂度为 O(logN)
        调用入口, change(1,x,v);

todo
 线段树的区间查询
    ask:
        区间查询是一条形如"Q l r"的指令, 例如查询序列 A在区间[l,r]上的最大值,
即max(l<=i<=r){A[i]}: 我们只需要从根节点开始, 递归执行一下过程:
...
void build(int p, int l, int r) {
    ...
    pushup(p);  //从下往上传递信息, 可以省略这个函数写成一条代码  t[p].sum =
                // t[p<<1,l].sum+t[p<<1|1].sum;
}
